  ...
  if [ -z "$FOUND_INSTALL" ]; then
    ...
    if [ -n "$ZIP_ENCONTRADO" ]; then
      ...
      if command -v unzip >/dev/null 2>&1 || apt install -y unzip >/dev/null 2>&1; then
        ...
        # Se o zip tiver uma única pasta por dentro (ex: "lyra-central-api/"),
        # extraímos o CONTEÚDO dela direto pro APP_DIR, em vez de aninhar
        # "APP_DIR/lyra-central-api/..." — evita path duplicado e feio.
        ITENS_TOPO=$(find "$EXTRACT_TMP" -mindepth 1 -maxdepth 1 -not -name '*.zip')
        QTD_ITENS=$(echo "$ITENS_TOPO" | grep -c .)
        if [ "$QTD_ITENS" -eq 1 ] && [ -d "$ITENS_TOPO" ]; then
          cp -a "${ITENS_TOPO}/." "$APP_DIR/"
        else
