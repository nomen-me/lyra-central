...
# ${DOMINIO_LYRA#*.} assume que sempre há um subdomínio (ex: "lyra.empresa.com"
# -> "empresa.com"). Se o domínio informado não tiver subdomínio (ex: só
# "empresa.com"), essa mesma expansão cortaria até o próximo ponto e sobraria
# só "com" — e-mail inválido pro Let's Encrypt. Só aplica o corte se sobrar
# pelo menos mais um ponto depois (ou seja, se DOMINIO_LYRA for mesmo um
# subdomínio); senão usa o domínio inteiro como está.
if [[ "$DOMINIO_LYRA" == *.*.* ]]; then
  EMAIL_DOMINIO_PADRAO="${DOMINIO_LYRA#*.}"
else
  EMAIL_DOMINIO_PADRAO="$DOMINIO_LYRA"
fi
EMAIL="${EMAIL:-admin@${EMAIL_DOMINIO_PADRAO}}"
...
if [ -d "${APP_DIR}/.git" ]; then
...
elif [ -f "${APP_DIR}/scripts/install.sh" ]; then
  ...
  # e não tem problema, porque atualizações de código são feitas via
  # scripts/deploy.sh, não por este script). Rodar "git clone" de novo
  # aqui só ia falhar (pasta não-vazia).
  ok "Código da Lyra Central já está em ${APP_DIR} de uma execução anterior — mantendo. Pra atualizar o código, use scripts/deploy.sh (ver resumo final)."
else
...
if [ ! -f "${APP_DIR}/scripts/install.sh" ]; then
  ...
  CANDIDATO="$(find "$APP_DIR" -maxdepth 4 -path '*/scripts/install.sh' 2>/dev/null | head -1)"
    ...
    if [ -n "$ZIP_ENCONTRADO" ]; then
      ...
      if command -v unzip >/dev/null 2>&1 || apt install -y unzip >/dev/null 2>&1; then
        ...
        if [ -n "$CANDIDATO" ]; then
          ...
          CANDIDATO="$APP_DIR/scripts/install.sh"  # já achatado, path final
        fi
        rm -rf "$EXTRACT_TMP"
      fi
    fi
  ...
  # ${APP_DIR}/scripts/install.sh, um nível só, sem exceção. Isso elimina
  # o "path duplicado" de vez, seja qual for a origem da bagunça.
  if [ -n "$CANDIDATO" ]; then
    RAIZ_REAL="$(dirname "$(dirname "$CANDIDATO")")"
    if [ "$RAIZ_REAL" != "$APP_DIR" ]; then
      warn "scripts/install.sh está aninhado em '${RAIZ_REAL}' — achatando pra '${APP_DIR}' direto."
      FLAT_TMP="$(mktemp -d)"
      cp -a "${RAIZ_REAL}/." "$FLAT_TMP/"
      rm -rf "$APP_DIR"; mkdir -p "$APP_DIR"
      cp -a "${FLAT_TMP}/." "$APP_DIR/"
      rm -rf "$FLAT_TMP"
    fi
  fi

  FOUND_INSTALL="$([ -f "${APP_DIR}/scripts/install.sh" ] && echo "${APP_DIR}/scripts/install.sh")"

  if [ -n "$FOUND_INSTALL" ]; then
    ok "scripts/install.sh confirmado em ${APP_DIR}/scripts/install.sh (achatado, sem aninhamento)"
  else
...
# (todo este script exige root, ver checagem no topo), não como ${APP_USER}.
# Se ele criar a pasta data/ nesse meio-tempo (ex: durante "npm ci" ou no
# primeiro boot da API pra preparar tenants.dev.json em SECRETS_PROVIDER=env),
# essa pasta nasce dona de root — por cima do chown que já fizemos na Seção 2
# (que rodou ANTES de data/ existir, então não pegou essa pasta). Sem repetir
# o chown aqui, o serviço systemd (que roda como ${APP_USER}) recebe EACCES
# ao tentar abrir data/tenants.dev.json em qualquer chamada a /admin/tenants.
...
LYRA_TRAEFIK_DIR="${APP_HOME_DIR}/lyra-traefik"

# Se este script já rodou antes nesta VPS, o container se chama
# "lyra-traefik" e é seguro recriar (docker compose up -d é idempotente).
# Mas se NUNCA rodou aqui e já existe outro container publicando 80/443 —
# tipicamente o "traefik" do instalar_nomen_completo.sh, que já expõe esta
# mesma Lyra Central via /home/ubuntu/traefik/dynamic/lyra.yml — subir um
# SEGUNDO Traefik com network_mode: host ia disputar exatamente as mesmas
# portas e falhar (ou pior, derrubar o outro). Detecta isso e pula a seção
# em vez de tentar e quebrar.
CONFLITO_TRAEFIK=false
if ! docker inspect lyra-traefik >/dev/null 2>&1; then
  if ss -ltn 2>/dev/null | awk '{print $4}' | grep -qE '(^|:)(80|443)$'; then
    CONFLITO_TRAEFIK=true
  fi
fi

if [ "$CONFLITO_TRAEFIK" = true ]; then
  warn "As portas 80 e/ou 443 já estão em uso por outro processo/container nesta"
  warn "VPS (ex: o Traefik do instalar_nomen_completo.sh, que já expõe"
  warn "${DOMINIO_LYRA} via ${APP_HOME_DIR}/traefik/dynamic/lyra.yml). Subir um"
  warn "segundo Traefik aqui (network_mode: host) ia brigar pelas mesmas portas."
  warn "Pulando a seção de Traefik deste script — se o HTTPS de ${DOMINIO_LYRA}"
  warn "já está coberto pelo Traefik existente, está tudo certo; se não estiver,"
  warn "libere as portas 80/443 (pare o outro Traefik) e rode este script de novo."
else
  log "Configurando Traefik (HTTPS) pra ${DOMINIO_LYRA}..."
  mkdir -p "${LYRA_TRAEFIK_DIR}/dynamic"
  cat > "${LYRA_TRAEFIK_DIR}/dynamic/lyra.yml" << EOF
http:
  routers:
    lyra:
...
EOF
  ...
  cat > "${LYRA_TRAEFIK_DIR}/docker-compose.yml" << EOF
services:
  traefik:
    image: traefik:v3.6
...
EOF
  ...
  cd "${LYRA_TRAEFIK_DIR}" && docker compose up -d
